...
Exactly two actors retain real UI signup: the main customer lifecycle actor and cross-customer B, whose isolated registration contributes directly to the reviewed journey. Blocked-controls, cross-customer A, stale-state, restricted/account-state, and ambiguous-response actors are setup-only and use the local fixture controller. It calls the loopback Auth admin endpoint outside the browser to create a confirmed password user, waits for the unchanged Ticket 9B trigger postcondition, and then establishes a real browser session through the public anon client. The local fixture admin credential is parsed only from `supabase status` for the disposable process, validated as loopback/service-role, never included in runtime configuration, never passed to page code, and never printed. Restricted, suspended, closed, and provider cases sign in while still customer/active before the fixture controller changes protected profile state. The missing-profile case signs in normally before its synthetic profile is deliberately removed.

The provider case separates local Auth-user creation, Ticket 9B profile readiness, normal password-session readiness, protected role mutation, and route-guard verification. Its fixed redacted failure categories are `provider-signup-http-failure`, `provider-session-missing`, `provider-profile-readiness-timeout`, `provider-role-fixture-failure`, and `provider-guard-state-mismatch`. Before and after the guarded route, a no-output database invariant proves the actor has exactly one existing `provider`/`active` profile and no provider profile, provider service, customer request, exact-address row, bid, booking, payment, or identity-verification state.
