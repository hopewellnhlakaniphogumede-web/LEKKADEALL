...
The aborted-response scenario creates its own explicit browser context, synthetic account, and draft. A Chromium Fetch-domain interceptor pauses the original browser mutation at request stage, continues exactly the first request with response interception enabled, and aborts that same response only after observing a successful server status. Any second request is failed at request stage before it can reach the RPC. The interceptor remains active through the ambiguous UI and fresh-read checks. The test requires one request-stage interception, one response-stage interception, and one network-policy RPC count; proves `Draft updated.` never appears; proves the submit control remains disabled after the ambiguous result; and requires a new RLS-backed `service_requests` read before accepting the authoritative edited values. The existing 180-second bound, zero Playwright retries, privacy-safe reporter, and final lifecycle postconditions remain.
