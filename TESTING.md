...
The lifecycle identity is now composed from the GitHub workflow run ID, workflow attempt, a fresh runner nonce, a bounded test-name slug plus digest, the actor label, and a fresh actor suffix. Local runs substitute a process-scoped run component. The identity is never printed. Immediately before either authorized UI signup, the fixture controller requires the scalar state `absent`; it returns no row or credential. The browser helper still clicks `Create account` exactly once, while the application now rejects re-entrant Auth form submissions with a single-flight guard.

If that one signup request ends with a network ambiguity or conflict, the helper does not send another signup. It accepts reconciliation only when the identity was absent immediately before the request, exactly one signup request was observed, and the no-row classifier subsequently reaches `auth-only` or `ready`. Ticket 9B readiness still has an explicit 60-second bound. A reconciled identity without an SDK session signs in through the normal public anon client; a pre-existing identity, zero/multiple signup requests, an absent post-state, or an invalid fixture fails with a fixed privacy-safe category. No user is deleted and no unexpected account is adopted.

The aborted-response scenario now creates its own explicit browser context, synthetic account, and draft. Its route handler remains installed for the whole mutation window: the first request is executed with transport retries disabled, the response is aborted only after an OK execution result, and any second request is aborted before it can reach the RPC. The test requires one intercepted request and one network-policy RPC count, proves `Draft updated.` never appears, proves the submit control remains disabled after the ambiguous result, removes the interceptor, and then requires a new RLS-backed `service_requests` read before checking the authoritative edited values. The existing 180-second bound, zero Playwright retries, privacy-safe reporter, and final lifecycle postconditions remain.
