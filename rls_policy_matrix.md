...
**Date:** 12 July 2026  
**Ticket:** Ticket 2 baseline RLS + Ticket 5 exact-address privacy + Ticket 6 marketplace state-machine update + Ticket 7A payment ledger foundation + Ticket 7B refund ledger foundation + Ticket 7C cash-disabled MVP policy + Ticket 7D payout release controls
...
- Mock webhook state changes: only through `admin_process_verified_mock_payment_webhook(...)` after a future server route has already verified the signature. This function stores `payload_hash` and safe metadata only.
...
## Mock/sandbox webhook processing after Ticket 8A

- Ticket 8A implements database-side processing for verified mock/sandbox payment webhook events only. It does not add HTTP routes, Edge Functions, live provider webhooks, real webhook secrets, real card/EFT processing, or UI.
...
- Ticket 7A hardens payment status constraints and the payment event ledger. Ticket 7B adds refund request and refund event ledger foundations. Ticket 7C disables cash for MVP. Ticket 7D adds internal/manual-sandbox payout release controls and blocker checks. Ticket 7E adds mock/sandbox hosted checkout and mock payment outcome recording only. Ticket 8A adds mock/sandbox database-side webhook processing after signature verification is assumed complete. Real HTTP webhook routes, raw body signature verification, live provider webhook transitions, real hosted checkout funding, real provider refund execution, real provider payout execution, reconciliation, and the full dispute workflow remain future tickets.
