...
function failedSafePhase(steps = []) {
  for (const step of steps) {
    ...
    if (step.error && (SAFE_FAILURE_PHASE.test(step.title)
        || SAFE_PROVIDER_BIDDING_FAILURE.test(step.title))) return step.title;
  }
  return null;
}
