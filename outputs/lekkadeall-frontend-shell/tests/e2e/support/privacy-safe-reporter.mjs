const SAFE_FAILURE_PHASE = /^(?:registration-failure:(signup-http-409|signup-http-422|signup-http-429|signup-http-other|signup-session-missing|profile-readiness-timeout|signup-network-failure|signup-duplicate-request|signup-unexpected-collision|signup-reconciliation-invalid|signup-reconciliation-session-failure)|registration-phase:(identity-precondition|signup-request|auth-session|profile-ready|dashboard)|ambiguous-update-failure:(isolated-setup|single-update-execution|ambiguous-ui|interception-release|detail-navigation|fresh-rls-read|canonical-values|postcondition|sign-out|cleanup)|ambiguous-setup-failure:(browser-context|fixture-account|browser-session|draft-create|edit-route)|guard-state:(restricted|suspended|closed|provider|missing-profile)|negative-actor-phase:(restricted|suspended|closed|provider|missing-profile):(auth-creation|ticket-9b-profile-readiness|browser-session|fixture-state-application|route-guard-verification|sign-out)|negative-actor-failure:(restricted|suspended|closed|provider|missing-profile):(auth-creation|ticket-9b-profile-readiness|browser-session|fixture-state-application|route-guard-verification|sign-out)|lifecycle-phase:(registration|reauthentication|category-dashboard|create|list-detail|update|cancel|postcondition|sign-out))$/u;
