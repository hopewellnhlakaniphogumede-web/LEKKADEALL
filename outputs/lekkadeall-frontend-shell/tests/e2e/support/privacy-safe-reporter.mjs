const SAFE_FAILURE_PHASE = /^(?:registration-failure:(signup-http-429|signup-http-conflict|signup-http-other|signup-session-missing|profile-readiness-timeout|signup-network-failure)|registration-phase:(signup-request|auth-session|profile-ready|dashboard)|provider-failure:(provider-signup-http-failure|provider-session-missing|provider-profile-readiness-timeout|provider-role-fixture-failure|provider-guard-state-mismatch)|guard-state:(restricted|suspended|closed|provider|missing-profile)|guard-phase:(restricted|suspended|closed|provider|missing-profile):(registration|fixture|route|postcondition|sign-out)|lifecycle-phase:(registration|reauthentication|category-dashboard|create|list-detail|update|cancel|postcondition|sign-out))$/u;
